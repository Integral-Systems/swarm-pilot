            ...
            if(d?.service_spec?.Spec?.Mode) {
                ...
                if ('Replicated' in mode) {
                    ...
                    if (scale !== 0) {
                        ...
                        yield* Effect.logInfo(`No scaling needed for service ${d.usage.service_name}, current scaling is ${current_scaling}. CPU: ${cpu_percent}%|${cpuUsage}${cpuLimit}, Memory: ${memory_percent}%|${memoryUsage}/${memoryLimit}`);
                    }
                }
            }
        }
